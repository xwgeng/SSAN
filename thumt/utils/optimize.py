...
def create_train_op(loss, optimizer, global_step, params):
    with tf.name_scope("create_train_op"):
        grads_and_vars = optimizer.compute_gradients(
            loss, tf.trainable_variables(params.trainable_scope), colocate_gradients_with_ops=True)
        #grads_and_vars = optimizer.compute_gradients(
        #    loss, tf.trainable_variables(), colocate_gradients_with_ops=True)
        ...
        # Update variables
        grads_and_vars = list(zip(gradients, tf.trainable_variables(params.trainable_scope)))
