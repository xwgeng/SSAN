...
def restore_variables(checkpoint):
    ...
    ops.append(tf.assign_sub(tf.assign(global_step, values["global_step"]), 1))

    var_list = tf.trainable_variables()
    for var in var_list:
        name = var.name.split(":")[0]
        name = "transformer" + name.replace("rlsan", "")
...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        # Print parameters
        all_weights = {v.name: v for v in tf.trainable_variables(params.trainable_scope)}
        ...
        with tf.train.MonitoredTrainingSession(
            ...
            utils.session_run(sess, restore_op)
